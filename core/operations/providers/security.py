    ...
    def _fetch_data(self) -> ProviderResult:
        ...
        if not getattr(settings, 'SECURE_SSL_REDIRECT', False):
            if not settings.DEBUG:
                status = "warning"
                checks.append(CheckResult(name="SSL Redirect", status="warning", message="Disabled"))
            else:
                checks.append(CheckResult(name="SSL Redirect", status="warning", message="Disabled (Dev)"))
        else:
            checks.append(CheckResult(name="SSL Redirect", status="healthy", message="Enabled"))
            
        if not getattr(settings, 'SESSION_COOKIE_SECURE', False):
            if not settings.DEBUG:
                status = "warning"
                checks.append(CheckResult(name="Secure Cookies", status="warning", message="Disabled"))
            else:
                checks.append(CheckResult(name="Secure Cookies", status="warning", message="Disabled (Dev)"))
        else:
            checks.append(CheckResult(name="Secure Cookies", status="healthy", message="Enabled"))
        ...
        if recent_security_events > 0:
            ...
            checks.append(CheckResult(name="Recent Threats", status="warning", message=f"{recent_security_events} alerts logged"))
