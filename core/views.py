...
from .models import ToolRun, ToolRunFile, CofWorkbook, BtplWorkbook, AttendanceWorkbook, FtlWorkbook
    ...
    if ftl_file_path and os.path.exists(ftl_file_path):
        try:
            ...
            if ftl_sheet_name in wb_ftl.sheetnames:
                ...
                mapping_ftl = ftl.get_column_mapping(sheet_ftl)
                vendors_set = set()
                ...
                for r in range(2, sheet_ftl.max_row + 1):
                    ...
                    status = ftl.derive_status(etd_val, del_val)
                    if status == 'Delivered':
                        ftl_delivered += 1
...
def ftl_sheet(request):
    ...
    page = int(request.GET.get('page', 1))
    page_data = ftl.get_ftl_page_data(
        file_path, sheet_name=sheet_name, page=page, page_size=20
    )
...
def ftl_api(request):
    ...
    if action == 'get_row':
        ...
        if row_num < 2:
            return JsonResponse({'error': 'Invalid row'}, status=400)
        row_data = ftl.get_ftl_row_values(file_path, row_num, sheet_name=sheet_name)
    ...
    if action == 'preview':
        page = int(request.GET.get('page', 1))
        preview = ftl.get_ftl_preview(file_path, sheet_name=sheet_name, page=page, page_size=20)
        return JsonResponse({'preview': preview})
    ...
    if action == 'save' and request.method == 'POST':
        ...
        if form.is_valid():
            ...
            target_row = row_data['row_num']
            try:
                ftl.add_ftl_shipment(file_path, row_data, sheet_name=sheet_name)
                run = ToolRun.objects.create(
                    user=request.user,
    ...
    if action == 'delete' and request.method == 'POST':
        row_num = int(request.POST.get('row', 0))
        page_data = ftl.get_ftl_page_data(file_path, sheet_name=sheet_name, page=1, page_size=1)
        totals_row = page_data['totals_row'] if page_data else 1000
        if row_num < 2 or row_num >= totals_row:
            return JsonResponse({'error': 'Invalid row number'}, status=400)
        try:
            ftl.clear_ftl_row(file_path, row_num, sheet_name=sheet_name)
            ToolRun.objects.create(
                user=request.user,
                tool=ToolRun.TOOL_FTL,
                status=ToolRun.STATUS_SUCCESS,
    ...
    # GET: next available row
    if action == 'next_row':
        next_row = ftl.find_next_ftl_row(file_path, sheet_name=sheet_name)
        return JsonResponse({'next_row': next_row})
